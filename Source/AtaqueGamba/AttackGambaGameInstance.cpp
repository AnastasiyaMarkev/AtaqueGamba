...
void UAttackGambaGameInstance::InitializeDefaultSaveData()
{
    ...
    SavedGameData.CurrentLevel = TEXT("");
...
{
    UAttackGambaSaveGame* SaveObject = Cast<UAttackGambaSaveGame>(
...
bool UAttackGambaGameInstance::LoadFromDisk(const FString& SlotName)
{
    ...
    UAttackGambaSaveGame* SaveObject = Cast<UAttackGambaSaveGame>(Loaded);
    ...
    if (SavedGameData.PlayerHealth <= 0.f)
    {
        SavedGameData.PlayerHealth = 100.f;
    }
...
bool UAttackGambaGameInstance::ApplyPendingPunishment()
{
    ...
    SavedGameData.MayonnaiseAmmo = FMath::Max(0, SavedGameData.MayonnaiseAmmo - PunishmentAmmoLoss);
    SavedGameData.bPunishmentPending = false;
